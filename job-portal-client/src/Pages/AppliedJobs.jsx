...
import { FiCalendar, FiClock, FiDollarSign, FiMapPin, FiTrash2 } from "react-icons/fi";
...
const AppliedJobs = () => {
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:3000/applied-jobs/${encodeURIComponent(user.email)}`);
        if (response.ok) {
  ...
  const handleWithdrawApplication = async (jobId, jobTitle) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        const response = await fetch(`http://localhost:3000/withdraw-application`, {
          method: 'DELETE',
          headers: {
        ...
        Swal.fire('Error', 'Failed to withdraw application. Please try again.', 'error');
      } finally {
        setWithdrawing(null);
      }
    }
    ...
    return (
      <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4 py-8">
        <div className="text-center">
          <h1 className="text-2xl font-bold mb-4">Please log in to view your applied jobs</h1>
    ...
    <div className="max-w-screen-2xl container mx-auto xl:px-24 px-4 py-8">
      ...
      {appliedJobs.length === 0 ? (
        <div className="text-center py-12">
          <div className="text-gray-500 text-lg mb-4">You haven't applied for any jobs yet</div>
          <Link 
            to="/" 
        ...
        <div className="space-y-6">
          {appliedJobs.map((application) => (
            <div key={application._id} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        <h3 className="text-xl font-semibold text-gray-800">
                          {application.jobDetails?.jobTitle || 'Job Title Not Available'}
                        </h3>
                      ...
                      <h4 className="text-lg text-blue-600 mb-3">
                        {application.jobDetails?.companyName || 'Company Name Not Available'}
                      </h4>
                ...
                <div className="flex gap-2 ml-4">
                  ...
                  <button
                    onClick={() => handleWithdrawApplication(application.jobId, application.jobDetails?.jobTitle)}
                  ...
                  >
                    <FiTrash2 size={14} />
                    {withdrawing === application.jobId ? 'Withdrawing...' : 'Withdraw'}
                  </button>
                </div>
              </div>
            </div>
